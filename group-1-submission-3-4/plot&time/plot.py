...
axs[0].set_yticks([0.5, 2.5, 4.5])
axs[0].set_yticklabels(['Node A', 'Node B', 'Node C'])
axs[0].set_title('Run1: Task Execution Timeline')

with open('measure1.txt', 'r') as f:
    lines = f.readlines()
...
axs[1].bar(times, latencies, color='black', edgecolor='black')
axs[1].set_xlabel('Seconds since first task started')
axs[1].set_ylabel('p95 Latency (ms)')
axs[1].set_title('Run1: Tail Latency Over Time')
axs[1].set_ylim(0, 1.5)
axs[1].axhline(y=1.0, color='r', linestyle='--', label='SLO')
